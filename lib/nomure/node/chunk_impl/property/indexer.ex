defmodule Nomure.Node.ChunkImpl.Property.Indexer do
  ...
  defp get_indexable_properties(node_name, node_data) do
    ...
    _get_indexable_properties(schema, node_name, node_data)
  end

  defp _get_indexable_properties(nil, _node_name, node_data) do
    node_data
    |> Enum.reduce(%{}, fn
  ...
  defp _get_indexable_properties(schema, node_name, node_data) do
    ...
    |> Enum.reduce(%{}, fn
      {key, value}, acc ->
        ...
        get_index_definition(node_schema, key, value)
        |> case do
          nil ->
            acc
  ...
  defp get_index_definition(nil, _key, _value) do
    nil
  end

  defp get_index_definition(node_schema, key, value) do
    case node_schema[key] do
      nil ->
        nil
