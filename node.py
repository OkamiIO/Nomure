...
class Graph:
    ...
    def __init__(self, directory, attributes, name):
        self.name = name
    ...
    def get(self, tr, uid, fields):
        ...
        """
        if uid is None or not self.__props_exists(fields, self.properties):
            return None
    ...
    def get_data(self, tr, uid, fields):
        ...
        for property_name, value in fields.items():
            if property_name == '__name__':
                ...
                result[property_name] = tr[self.properties[uid][property_name.encode()]]
        return result
...
class Node(Graph):
    ...
    def __set_node_property(self, tr, value, edge_name, uid):
        ...
        if not value[e_key] and self.properties[edge_name].edge_properties != {}:
            return False

        # Set the edge data
        edge_data = value[e_key]
        edge = Edge(self.properties[edge_name].edge_properties, edge_name)
        ...
        relation_node = Node.get_node_by_name(self.properties[edge_name].node_name)
        if relation_node is None:
            return False
    ...
    def set_data(self, tr, values):
        ...
        for edge_name, value in values.items():
            ...
            if self.properties[edge_name].kind == PropertyKind.NODE:
                ...
                if NodePropertyIndexes.UNIQUE in self.properties[edge_name].indexes:
    ...
    def get_all(self, tr, node_uid):
        ...
        if node_uid is None:
            return None
        result = {}
        for edge_name, edge_properties in self.properties.items():
    ...
    def get_data(self, tr, node_uid, fields):
        ...
        """
        result = self.__get_node_fields(tr, node_uid, fields, self.properties, self.edges)
