...
class GraphEntity:
    ...
    # Custom properties
    # __name__ returns the given object name, useful for Union types in GraphQL

    def __init__(self, directory, schema, name):
        self.name = name
        self.schema = schema
        self.directory = directory
        self.root = directory[name]
        self.properties = self.root['P']
    ...
    def set(self, tr, data):
        """ Set the given data into the database

        :param tr: Database instance
        :param data: Dictionary describing the data
        :return: Unique identifier
        """
        for att_name, value in data.items():
            if self.__check_props_values(att_name, value):
                continue
            return None
        return self.set_data(tr, data)

    def get(self, tr, uid, fields):
        """ Get the node specified fields (edges) and field child

        :param tr: Database instance
        :param uid: Database Unique identifier
        :param fields: Edges to be query
        :return: The result of the transaction
        """
        if uid is None or not self.__props_exists(fields, self.schema):
            return None
        return self.get_data(tr, uid, fields)
    ...
    @fdb.transactional
    def get_data(self, tr, uid, fields):
        """ Must be override and implemented, get the data in the database by the given fields

        :param tr: Database instance
        :param uid: Database unique identifier
        :param fields: Dictionary describing the properties and data to be query
    ...
    def set_property(self, tr, uid, property_name, value):
    ...
    def __check_props_values(self, key, value):
        ...
        if key not in self.schema:
            return False
        elif self.__is_valid_value(key, value):
            return True
        else:
            return False
    ...
    def __props_exists(self, fields, props):
        ...
        for k, v in fields.items():
            ...
            # is not exposed in the properties but exist internally
            if k == 'uid' or k == '__name__':
                continue
            if k not in props:
                return False
            if isinstance(props[k], WithNodeProperty):
                return self.__props_exists(v, Node.schemas[props[k].node_name])
        return True
...
class Edge(GraphEntity):
    ...
    def __init__(self, properties, edge_name):
        super().__init__(Edge.directory, properties, edge_name)

    @fdb.transactional
    def set_data(self, tr, data):
        uid = self._set_uid(tr)
        ...
        if data is not None:
            for property_name, value in data.items():
                ...
                self.set_property(tr, uid, property_name, fdb_value)
        return uid
    ...
    def get_data(self, tr, uid, fields):
        ...
        for property_name, value in fields.items():
            if property_name == '__name__':
                ...
                result[property_name] = tr[self.properties[uid]
...
class Node(GraphEntity):
    """
    Represents a Node (Edge) in the graph, it contains the properties and values as well
    a predefined schema
    """
    db = database
    directory = fdb.directory.create_or_open(db, ('graph',))
    reserved_edge_names = ['uid', '__name__']

    # Begin as an empty dict, once the schema is parsed must be filled with the schemas info
    schemas = {}

    def __init__(self, node_name, schema):
        super().__init__(Node.directory, schema, node_name)
        # inverse instance
        self.inverse = self.root['I']
        # indexes
        self.indexes = self.root['IN']
        # Set the schema in the global schema storage
        if node_name in Node.schemas or node_name in NodeStorage.nodes:
            raise KeyError(
                "The given node_name is already set in the schema, please check duplication issues")
        Node.schemas[node_name] = schema
        NodeStorage.nodes[node_name] = self

    def __get_node_fields(self, tr, node_uid, fields, props, node_property):
        ...
        for property_name, value in fields.items():
            if property_name == '__name__':
                ...
                for relation_edge_uid, relation_node_uid in tr[node_property[node_uid][property_name.encode()].range()]:
                    ...
                    relation_node = Node.get_node_by_name(
                        props[property_name].node_name)

                    # TODO reset or ignore the field?
                    if relation_node is None:
                        return None

                    relation_node_fields = self.__get_node_fields(
                        tr, relation_node_uid, value, node_props, relation_node.properties)

                    if relation_node_fields is None or relation_node_fields == {}:
                        return None

                    result_nodes.append(relation_node_fields)
                result[property_name] = result_nodes
            else:
                # is a node_property
                result[property_name] = tr[node_property[node_uid]
                                           [property_name.encode()]]
        return result

    def __set_node_property(self, tr, value, edge_name, uid):
        ...
        if not value[e_key] and self.schema[edge_name].edge_properties != {}:
            return False

        # Set the edge data
        edge_data = value[e_key]
        edge = Edge(self.schema[edge_name].edge_properties, edge_name)
        ...
        relation_node = Node.get_node_by_name(self.schema[edge_name].node_name)
        if relation_node is None:
            return False
        ...
        if isinstance(node_data, bytes):
            ...
            if not tr[relation_node.properties[node_data][b'uid']].present():
                return False
            tr[self.properties[uid][edge_name.encode()][edge_uid]] = node_data
            tr[self.inverse[node_data][edge_name.encode()][uid]] = edge_uid
            tr[self.indexes[edge_name.encode()][uid][node_data]] = b''
        elif isinstance(node_data, dict):
            relation_uid = relation_node.set(tr, node_data)
            if relation_uid is None:
                return False
            tr[self.properties[uid][edge_name.encode()][edge_uid]] = relation_uid
            tr[self.inverse[relation_uid][edge_name.encode()][uid]] = edge_uid
            tr[self.indexes[edge_name.encode()][uid][relation_uid]] = b''
        else:
            return False
        return True
    ...
    def set_data(self, tr, values):
        ...
        for edge_name, value in values.items():
            ...
            if self.schema[edge_name].kind == PropertyKind.NODE:
                if self.__set_node_property(tr, value, edge_name, uid) is False:
                    return self.reset_transaction(tr)
            else:
                # Get the fbd version of the given value
                fdb_value = self.get_as_fdb_value(value)
                ...
                if NodePropertyIndexes.UNIQUE in self.schema[edge_name].indexes:
                    # TODO there must be a better way to handle this validation
                    tr_range = self.indexes[edge_name.encode(
                    )][fdb_value].range()
                ...
                self.set_property(tr, uid, edge_name, fdb_value)
        return uid
    ...
    def get_all(self, tr, node_uid):
        ...
        if node_uid is None:
            return None
        result = {}
        for edge_name, edge_properties in self.schema.items():
            if edge_properties.kind == PropertyKind.NODE:
                # TODO query edge info
                node_result = []
                for _, relation_node_uid in tr[self.properties[node_uid][edge_name.encode()].range()]:
                    node_result.append(relation_node_uid)
                edge_value = node_result
            else:
                edge_value = tr[self.properties[node_uid][edge_name.encode()]]
            result[edge_name.encode()] = edge_value
        if result == {}:
            return None
        result[b'uid'] = tr[self.properties[node_uid][b'uid']]
        return result

    @fdb.transactional
    def get_data(self, tr, node_uid, fields):
        ...
        """
        result = self.__get_node_fields(
            tr, node_uid, fields, self.schema, self.properties)
    ...
    def get_edge_value(self, tr, node_uid, edge_name):
        ...
        """
        if self.schema[edge_name].kind == PropertyKind.NODE:
            return [v for _, v in tr[self.properties[node_uid][edge_name.encode()].range()]]
        return tr[self.properties[node_uid][edge_name.encode()]]

    @fdb.transactional
    def get_by(self, tr, edge_name, value, limit=20):
        """ Get the nodes edges based on a edge property value

        :param limit: The limit of items to be query, default 20
        :param tr: Database instance
        ...
        result = [self.get_all(tr, self.indexes.unpack(k)[-1])
                  for k, _ in tr.get_range(tr_range.start, tr_range.stop, limit=limit)]
        if not result:
            return None
        return result
